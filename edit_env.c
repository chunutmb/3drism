...
void print_env( char outfile[] )
{
	...
	FILE *out;
		...
		if( strncmp( "CY", s1 , 2) == 0 ){ scanf( "%d", &d);  	fprintf( out, "#CY\t\t%d\n", d ); }
		  else							fprintf( out, "#CY\t\t%d\n", CY );

		if( strncmp( "CZ", s1 , 2) == 0 ){ scanf( "%d", &d);  	fprintf( out, "#CZ\t\t%d\n", d ); }
		  else							fprintf( out, "#CZ\t\t%d\n", CZ );

		if( strncmp(         "A_ERF", s1 , 5) == 0 ){ scanf( "%lf", &f);  fprintf( out, "#A_ERF        \t%f\n",  f ); }
		  else								  fprintf( out, "#A_ERF        \t%f\n", A_ERF );

		if( strncmp(      "CHRG_PCT", s1 , 8) == 0 ){ scanf( "%lf", &f);  fprintf( out, "#CHRG_PCT     \t%f\n",  f ); }
		  else								  fprintf( out, "#CHRG_PCT     \t%f\n", CHRG_PCT );

		if( strncmp(       "CLOSURE", s1 , 7) == 0 ){ scanf(  "%s", s2);  fprintf( out, "#CLOSURE      \t%s\n", s2 ); }
		  else								  fprintf( out, "#CLOSURE      \t%s\n", CLOSURE );

		if( strncmp(   "CONFIG_TYPE", s1 ,11) == 0 ){ scanf(  "%s", s2);  fprintf( out, "#CONFIG_TYPE  \t%s\n", s2 ); }
		  else								  fprintf( out, "#CONFIG_TYPE  \t%s\n", CONFIG_TYPE );

		if( strncmp(      "CONTINUE", s1 , 8) == 0 ){ scanf(  "%s", s2);  fprintf( out, "#CONTINUE     \t%s\n", s2 ); }
		  else								  fprintf( out, "#CONTINUE     \t%s\n", CONTINUE );

		if( strncmp(     "DIIS_SIZE", s1 , 9) == 0 ){ scanf(  "%d", &d);  fprintf( out, "#DIIS_SIZE    \t%d\n", d ); }
		  else								  fprintf( out, "#DIIS_SIZE    \t%d\n", DIIS_SIZE );

		if( strncmp(       "DIIS_MP", s1 , 7) == 0 ){ scanf(  "%d", &f);  fprintf( out, "#DIIS_MP      \t%f\n", f ); }
		  else								  fprintf( out, "#DIIS_MP      \t%f\n", DIIS_MP );

		if( strncmp(    "EWALD_SUMS", s1 ,10) == 0 ){ scanf(  "%s", s2);  fprintf( out, "#EWALD_SUMS   \t%s\n", s2 ); }
		  else								  fprintf( out, "#EWALD_SUMS   \t%s\n", EWALD_SUMS );

		if( strncmp(   "GAUSS_SMEAR", s1 ,11) == 0 ){ scanf( "%lf", &f);  fprintf( out, "#GAUSS_SMEAR  \t%f\n", f ); }
		  else								  fprintf( out, "#GAUSS_SMEAR  \t%f\n", GAUSS_SMEAR );

		if( strncmp( "INIT_PIC_ITER", s1 ,13) == 0 ){ scanf(  "%d", &d);  fprintf( out, "#INIT_PIC_ITER\t%d\n", d ); }
		  else								  fprintf( out, "#INIT_PIC_ITER\t%d\n", INIT_PIC_ITER );

		if( strncmp(      "MAX_ITER", s1 , 8) == 0 ){ scanf(  "%d", &d);  fprintf( out, "#MAX_ITER     \t%d\n", d ); }
		  else								  fprintf( out, "#MAX_ITER     \t%d\n", MAX_ITER );

		if( strncmp(        "N_DUMP", s1 , 6) == 0 ){ scanf(  "%d", &d);  fprintf( out, "#N_DUMP       \t%d\n", d ); }
		  else								  fprintf( out, "#N_DUMP       \t%d\n", N_DUMP );

		if( strncmp(        "PIC_MP", s1 , 6) == 0 ){ scanf( "%lf", &f);  fprintf( out, "#PIC_MP       \t%f\n", f ); ; }
		  else								  fprintf( out, "#PIC_MP       \t%f\n", PIC_MP );

		if( strncmp(        "SOLVER", s1 , 6) == 0 ){ scanf(  "%s", s2);  fprintf( out, "#SOLVER       \t%s\n", s2 ); }
		  else								  fprintf( out, "#SOLVER       \t%s\n", SOLVER );

		if( strncmp(         "T_ERR", s1 , 5) == 0 ){ scanf( "%lf", &f);  fprintf( out, "#T_ERR        \t%f\n", f ); }
