...
double get_dval(char in_file[], char val[])
{
  ...
  while (!feof(in)) {
    ...
    if (c1 != '#') {
      idx = 0;
      do {
        s1[idx] = c1;                           /*get full variable name*/
        idx++;
        c1 = fgetc(in);
      } while ((c1 != ' ') && (c1 != '\n') && (c1 != '\t') && (c1 != '\r'));
      s1[idx] = '\0';
...
int get_tag(char in_file[], char val[])
{
  ...
  while (!feof(in)) {
    ...
    if (c1 != '#') {
      ...
      if (strncmp(val, s1, idx) == 0) {
        test1 = 1;
      } else if ((strncmp("DIS", s1, idx) == 0) || (strncmp("END", s1, idx) == 0)) {
        break;
      }
    }
    if (test1 == 1) break;
...
double * get_dis(char in_file[], int col)
{
  ...
  while (!feof(in)) {
    ...
    if (c1 != '#') {
      ...
      if (strncmp("N_PTS", s1, idx) == 0) {
        fscanf(in, "%d", &n_pts);
      } else if (strncmp("RADIUS", s1, idx) == 0) {
        fscanf(in, "%lf", &radius);
      } else if (strncmp("DIS_NUM", s1, 7) == 0) {
        fscanf(in, "%d", &dis_num);
      } else if (strncmp("DIS", s1, idx) == 0) {
